...
import { buscarHorasMaquina } from "@/lib/relatorios";
...
export default async function RelatorioHorasMaquinaPage({ searchParams }: { searchParams: Promise<Filtros> }) {
  ...
  const [talhoes, tiposAtividade, culturas, { linhas, total }] = await Promise.all([
    ...
    buscarHorasMaquina(propriedadeId, {
      ...filtros,
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-xl font-semibold text-neutral-900">Relatório de Horas de Máquina</h1>
      ...
      <div className="flex items-center justify-between rounded-xl border border-neutral-200 bg-white p-4">
        <div>
          <p className="text-xs text-neutral-500">Total de horas de máquina</p>
          <p className="text-2xl font-semibold text-neutral-900">{total.toLocaleString("pt-BR")}h</p>
        </div>
        <ExportarBotoes recurso="horas-maquina" filtros={queryString} />
      </div>
