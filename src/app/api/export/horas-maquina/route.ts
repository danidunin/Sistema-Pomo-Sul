...
export async function GET(request: Request) {
  ...
  linhasRelatorio.push({ data: null, origem: null, descricao: null, talhao: "Total", horas: total });

  return respostaRelatorio(
    formato,
    "horas-maquina",
    `${propriedade?.nome ?? "Propriedade"} — Horas de Máquina`,
    [
      { chave: "data", titulo: "Data" },
      { chave: "origem", titulo: "Origem" },
      { chave: "descricao", titulo: "Descrição", largura: 26 },
      { chave: "talhao", titulo: "Talhão", largura: 22 },
      { chave: "horas", titulo: "Horas" },
    ],
    linhasRelatorio,
  );
}
